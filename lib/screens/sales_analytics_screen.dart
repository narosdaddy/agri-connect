...
class _SalesAnalyticsScreenState extends State<SalesAnalyticsScreen> {
  ...
  List<Map<String, dynamic>> _getTopProducts() {
    ...
    final sortedProducts = _myProducts.where((p) => productSales.containsKey(p['id'])).toList();
    ...
    return sortedProducts.take(5).map((product) {
      return {
        ...product,
        ...
        children: [
          ...
          Row(
            children: [
              ...
              const SizedBox(width: 8),
              Expanded(
                child: _buildPeriodButton('year', 'Année'),
              ),
            ],
          ),
        ],
            ...
            children: [
              ...
              Expanded(
                child: _buildRevenueCard(
                  ...
                  value: '${_myProducts.where((p) => (p['quantity'] ?? 0) > 0).length}',
                  subtitle: 'En stock',
                  color: Colors.purple,
                ),
              ),
  ...
  Widget _buildProductPerformance() {
    final lowStockProducts = _myProducts.where((p) => (p['quantity'] ?? 0) < 10).toList();
    final outOfStockProducts = _myProducts.where((p) => (p['quantity'] ?? 0) == 0).toList();
